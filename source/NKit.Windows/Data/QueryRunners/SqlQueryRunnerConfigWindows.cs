namespace NKit.Data.QueryRunners
{
    ...
    [Serializable]
    public class SqlQueryRunnerConfigWindows
    {
        #region Constructors

        public SqlQueryRunnerConfigWindows(
            ...
            set
            {
                if(string.IsNullOrEmpty(value))
                {
                    throw new NullReferenceException(string.Format(
                        ...
                        EntityReaderGeneric<SqlQueryRunnerConfigWindows>.GetPropertyName(p => p.SqlQueryRunnerAssemblyPath, false),
        ...
        public string SqlQueryRunnerFullTypeName
        {
            ...
            set
            {
                if (string.IsNullOrEmpty(value))
                {
                    throw new NullReferenceException(string.Format(
                        "{0} may not be null or empty on {1}.",
                        EntityReaderGeneric<SqlQueryRunnerConfigWindows>.GetPropertyName(p => p.SqlQueryRunnerFullTypeName, false),
                        this.GetType().FullName));
                }
            ...
            set 
            {
                if (value == null || value.Length < 1)
                {
                    throw new NullReferenceException(string.Format(
                        ...
                        EntityReaderGeneric<SqlQueryRunnerConfigWindows>.GetPropertyName(p => p.QueryRunnerAssemblyBytes, false),
                        this.GetType().FullName));
                }
