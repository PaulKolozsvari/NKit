namespace NKit.Data.DB.SQLServer
{
    ...
    public static class SqlConnectionExtensionsCore
    {
        #region Constants
        ...
        /// <returns></returns>
        public static EntityCacheGeneric<string, ForeignKeyInfoCore> GetTableForeignKeys(this SqlConnection connection, string tableName)
        {
            string commandText = GetTableForeignKeysCommandText(tableName);
            using (SqlCommand command = new SqlCommand(commandText, connection))
            {
                command.CommandType = System.Data.CommandType.Text;
                command.Parameters.Add(new SqlParameter(string.Format("@{0}", TABLE_NAME_SQL_PARAMETER_NAME), tableName));
        ...
        private static EntityCacheGeneric<string, ForeignKeyInfoCore> GetForeignKeysFromSqlDataReader(SqlDataReader reader)
        {
            EntityCacheGeneric<string, ForeignKeyInfoCore> result = new EntityCacheGeneric<string, ForeignKeyInfoCore>();
            while (reader.Read())
            {
                ForeignKeyInfoCore f = new ForeignKeyInfoCore()
                {
